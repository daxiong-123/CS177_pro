...
LF_map::LF_map(int str_len, string s):cap(str_len),less_A(0),less_C(0),less_G(0),less_T(0){
    A = new int [str_len];
    T = new int [str_len];
    C = new int [str_len];
    G = new int [str_len];
...
int LF_map::get_less(char mode){
    ...
    else if(mode == 'T') return less_T;
    else if(mode == 'C') return less_C;
    else if(mode == 'G') return less_G;
}